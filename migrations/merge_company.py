...
class CompanyMergeMigrator(BaseUseCase):
    ...
    def merge_company_recruiters(self, from_company_id: str, to_company_id: str):
        ...
        print(
            f"Recruiters from company {from_company_id}: {len(all_from_company_recruiters)}"
        )
        for from_recruiter in all_from_company_recruiters:
            # Check that user id of from recruiter doesnt exist in any to recruiters
            if from_recruiter.user_id in [
                recruiter.user_id for recruiter in all_to_company_recruiters
            ]:
                continue
    ...
    def merge_company_applications(self, from_company_id: str, to_company_id: str):
        ...
        all_from_company_applications = application_repo.get_all(
            company_id=from_company_id
        )
        print(
            f"Applications from company {from_company_id}: {len(all_from_company_applications)}"
        )
    ...
    def merge_all_resumes(self, from_company_id: str, to_company_id: str):
        ...
        print(
            f"Resumes from company {from_company_id}: {len(all_from_company_resumes)}"
        )
    ...
    def merge_company_data(self, from_company_id: str, to_company_id: str):
        ...
        company_counts_migration = CompanyCountsMigrationUseCase(
            MIGRATION_REQUEST_SCOPE
        )
