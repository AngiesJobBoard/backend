...
class ApplicationFailureReRunUseCase(BaseUseCase):
    ...
    async def rerun_failed_application_match(self):
        ...
        for application in failed_application_matches:
            print(f"Trying to resubmit application for {application.id} - {application.name}")
            async with aiohttp.ClientSession() as session:
                try:
                    ...
                    print(f"Successfully resubmitted application for {application.id} - {application.name}!!\n")
                except Exception as e:
                    print(f"Failed to resubmit application for {application.id} - {application.name}: {e}\n")
                    continue
