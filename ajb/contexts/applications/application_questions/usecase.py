    ...
    async def update_application_with_questions_answered(
        ...
        for idx, question in enumerate(application.application_questions):
            if question.question_status == QuestionStatus.PENDING_ANSWER:
                try:
                    updated_application_question = await question_answerer.answer_question(
                        question.question, application.qualifications
                    )
                    application.application_questions[idx] = updated_application_question
        ...
        application_repo.update(
            application.id,
            UpdateApplication(**application.model_dump())
        )
        
        if stored_exception:
            raise stored_exception

    def update_many_application_questions(
