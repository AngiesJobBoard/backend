from ajb.contexts.interview.interview_questions.models import QuestionDocument
...
class InterviewQuestionRepository:
    ...
    def generate_interview_questions(
        ...
        system_prompt = """
        You are a perfect at assisting an interviewer with generating insightful questions. Using the given resume and job post listing, generate a suite of tailored interview questions that assess the interviewee's qualifications, experiences, and skills relevant to the job. Ensure the questions 
        are comprehensive and cover different aspects.
        """
        ...
        **Job Post Listing:**
        ...
        response = self.openai.structured_prompt(
            prompt=prompt,
            response_model=QuestionDocument,
            system_prompt=system_prompt,
            max_tokens=4096,
        )

        return print(response.model_dump_json())
...
Flexible Work Arrangements: Options for remote work and flexible scheduling."""
    ...
    repo.generate_interview_questions(resume_text, job_posting_text)
