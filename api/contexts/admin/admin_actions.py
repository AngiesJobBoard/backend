...
from ajb.contexts.users.actions.models import PaginatedUserActions
...
@router.get("/users", response_model=PaginatedUserActions)
def search_user_actions(
    request: Request,
    user_id: str | None = None,
    action_type: UserEvent | None = None,
    query: QueryFilterParams = Depends(),
):
    results = UserActionRepository(request.state.request_scope).query_event_type(
        user_id, action_type, query
    )
    return build_pagination_response(
        results, query.page, query.page_size, request.url._url, PaginatedUserActions
    )


@router.get("/users-timeseries")
def search_user_actions_timeseries(
    request: Request,
