...
from ajb.contexts.companies.api_egress_webhooks.repository import CompanyAPIEgressRepository


router = APIRouter(tags=["Company API Egress Webhooks"], prefix="/companies/{company_id}/api-egress-webhooks")
...
def get_all_company_egress_webhooks(
    ...
    response = CompanyAPIEgressRepository(request.state.request_scope, company_id).query(query)
    return build_pagination_response(
        response, query.page, query.page_size, request.url._url, PaginatedCompanyEgressWebhooks
    )


@router.post("/", response_model=CompanyAPIEgress)
def create_company_egress_webhook(request: Request, company_id: str, webhook: UserCreateCompanyAPIEgress):
...
def get_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(request.state.request_scope, company_id).get(webhook_id)
...
def disable_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(request.state.request_scope, company_id).update_fields(
...
def update_company_egress_webhook(request: Request, company_id: str, webhook_id: str, webhook: UpdateCompanyAPIEgress):
...
def delete_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(request.state.request_scope, company_id).delete(webhook_id)
