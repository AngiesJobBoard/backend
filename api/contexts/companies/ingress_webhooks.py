...
def get_all_company_ingress_webhooks(
    ...
    response = CompanyAPIIngressRepository(
...
def get_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIIngressRepository(request.state.request_scope, company_id).get(
        webhook_id
    )
...
def disable_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIIngressRepository(
        request.state.request_scope, company_id
...
def delete_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIIngressRepository(request.state.request_scope, company_id).delete(
        webhook_id
    )
