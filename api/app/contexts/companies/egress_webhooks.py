...
def get_all_company_egress_webhooks(
    ...
    response = CompanyAPIEgressRepository(scope(request), company_id).query(query)
...
def create_company_egress_webhook(
    ...
    return CompanyAPIEgressRepository(scope(request), company_id).create(
...
def get_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(scope(request), company_id).get(webhook_id)
...
def disable_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(scope(request), company_id).update_fields(
...
def update_company_egress_webhook(
    ...
    return CompanyAPIEgressRepository(scope(request), company_id).update(
        webhook_id, webhook
    )
...
def delete_company_egress_webhook(request: Request, company_id: str, webhook_id: str):
    return CompanyAPIEgressRepository(scope(request), company_id).delete(webhook_id)
